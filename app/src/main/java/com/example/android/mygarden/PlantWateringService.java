...
public class PlantWateringService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null) {
            ...
            final long id = intent.getLongExtra(EXTRA_PLANT_ID, PlantContract.INVALID_PLANT_ID);
    ...
    private void handleActionWaterPlant(long id) {
        Uri PLANT_URI = ContentUris.withAppendedId(PlantContract.PlantEntry.CONTENT_URI, id);
    ...
    private void handleActionUpdatePlantWidgets() {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            cursor.close();
            long lastWateredTime = timeNow - wateredAt;
            showWater = lastWateredTime > PlantUtils.MIN_AGE_BETWEEN_WATER &&
                    lastWateredTime < PlantUtils.MAX_AGE_WITHOUT_WATER;
